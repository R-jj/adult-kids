...
*
* Enhanced adult-kids with Camera, Button, and ASCII Art support
...
// Image dimensions (128x128 for adult-kids model)
#define IMAGE_SIZE_X (128)
#define IMAGE_SIZE_Y (128)

#define CAMERA_FREQ (5 * 1000 * 1000)

const char classes[CNN_NUM_OUTPUTS][12] = { "Adult", "Kids" };
...
int main(void)
{
    ...
    printf("\n\n=================================\n");
    printf("Adult-vs-Kids Feather Demo\n");
    printf("=================================\n\n");
    ...
    while (1) {
        ...
        if (result[0] == result[1]) {
            ...
            LED_On(LED1);  // Adult detected
            LED_Off(LED2);
        } else {
            printf("\nResult: %s detected with %d.%d%% confidence\n", 
                   classes[1], result[1], tens);
            LED_Off(LED1);
            LED_On(LED2);  // Kids detected
        }
